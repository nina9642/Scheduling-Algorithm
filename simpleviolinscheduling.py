...
def converting_masterclasses(simple_masterclasses):
    final = []
    for row in simple_masterclasses:
        row[0] = row[0]
        row[1] = int(row[1])
        student_group = []
...
def create_individual(simple_students, simple_teachers, simple_groupclasses):
    schedule = []
    masterclass = converting_masterclasses(simple_masterclasses)
    for row in masterclass:
        teacher_list = [teacher[0] for teacher in simple_teachers if teacher[2] >= row[1]]
        class_teacher = random.choice(teacher_list)
        time_list = [time[0] for time in simple_times]
        class_time = random.choice(time_list)
        class_schedule = (row[0], class_time, class_teacher, row[2])
        schedule.append(class_schedule)

    schedule_len = len(schedule)
    for row in simple_groupclasses:
        student_list = [student[0] for student in simple_students if student[2] == row[1]]
        for student in simple_students:
            if student[2] == row[2]:
                student_list.append(student[0])
        # print(student_list)
        teacher_list = [teacher[0] for teacher in simple_teachers 
...
file_path6 = 'Simple Students.csv'
file_path7 = 'Simple Teachers.csv'
file_path8 = 'Simple Masterclasses.csv'
file_path9 = 'Simple Times.csv'
file_path10 = 'Simple Group Classes.csv'
simple_students = read_csv_to_2d_list(file_path6)
for student in simple_students:
    student[2] = int(student[2])
simple_teachers = read_csv_to_2d_list(file_path7)
for teacher in simple_teachers:
    teacher[2] = int(teacher[2])
simple_masterclasses = read_csv_to_2d_list(file_path8)
for item in simple_masterclasses:
    item[1] = int(item[1])
simple_times = read_csv_to_2d_list(file_path9)
simple_groupclasses = read_csv_to_2d_list(file_path10)
for item in simple_groupclasses:
    item[1] = int(item[1])
    if item[2] != '':
        item[2] = int(item[2])
...
teacher_levels = dict()
for item in simple_teachers:
    teacher_levels[item[0]] = item[2]
#print(fitness(create_individual(simple_students,simple_teachers,simple_groupclasses), class_levels, teacher_levels))
print(fitness([('1', '1', '1', [1,2]), ('2', '2', '1', [3,4]), ('3', '2', '2', [5,6]),
               ('4', '1', '3', [7,8]), ('5', '3', '1', [1,2,3,4,5,6,7,8])], 
               class_levels, teacher_levels))
